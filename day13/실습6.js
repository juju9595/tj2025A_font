...
/*
// 메모(기억=변수) : 가장 큰 값
let max = numbers[0];   //첫번째 점수를 가장 큰 수로 기억/저장/변수기록
// 만약에 max(기록)보다 더 큰 수가 존재하면 더 큰수를 max(기록)
if(max < numbers[1]){max= unmbers[1]}
if(max < numbers[1]){max= unmbers[1]}
if(max < numbers[1]){max= unmbers[1]}
if(max < numbers[1]){max= unmbers[1]}
if(max < numbers[1]){max= unmbers[1]} // 반복적인 코드 확인
//
console.log(max.);
=====================================================================================
let numbers = [23, 5, 67, 12, 88, 34];
...
for( let index = 0; index <=numbers.length -1 ; index++){
    //index는 0부터 마지막인데스까지 이하까지 1씩증가.
    if(max < numbers[index]){max = numbers[index];}
    // 만약에 max보다 index번째 요소값이 더 크면 max에 index번째 요소값을 대입한다.
} //for end
console.log(max);
*/
...
=============================================================================
// 배열내 -> 한요소/값/문자열 -> 솔이 포함.//
//(1) 모든 배열의 이름들을 *하나씩: 0~ 마지막까지 1식* 꺼낸다. < 배열 순회>
let userNames = ['김하준', '이서아', '박솔민', '최도윤'];
...
let 수량 = Number(prompt(''))       //(2) 사용자로부터 구매할 수량 입력받아,
...
for(let index=0; index<=products.length -1; index++){
    ...
    if( product == 상품명 ){ // 만약에 index 번째 제품명과 입력 받은 제품명과 같으면
...
if(findIndex >= 0){
    if(stock[findIndex] >= 수량){//찾은 제품의 재고가 입력받은 수량보다 많으면
        console.log("구매완료!");
        stock[findIndex] -= 수량
    }else
}
...
for(let index=0 ; index <= seatStatus.length -1 ; index++){
    let seat=seatStatus[index]; // index 번째의 좌석을 호출
    if(seat== "빈좌석"){
