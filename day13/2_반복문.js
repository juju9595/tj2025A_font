...
for( let index = 0; index <= scores.length -1; index++){
    ...
    //let sum=0; 여기에 합게 변수를 선언하면 안된다 : 합계 변수는 *누적*해야하므로 선언을 반복하면 안됌.
    sum += scores[index]; // 배열내 index번재의 요소 값를 더한다.
}
...
let scores = [77, 82, 100, 54, 96];
for(let index= 0; index <= scores.length -1; index++){
    let value = scores[index]; // index번째
