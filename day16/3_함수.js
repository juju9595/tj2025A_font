...
*/
//(1)매개 변수 : x , y  : 매개 변수명은 아무거나.
//(2)반환값 : X
//(3)처리할명령어 : 더하기해서 console.log()
...
입력받은 이름을 매개변수로 받아, 해당 이름이 저장된 userList 전역 배열에 push하는 addUser 함수를 
정의하고 호출하시오.
*/
//(1)매개 변수 : name
//(2)반환값 : x
//(3)처리할명령어 : 매개 변수를 배열에 . push하는 addUser 함수를 만들어라
...
addUser(이름); // 입력받아 이름을 addUser 함수의 매개변수로 전달
...
*/
//(1)매개 변수(함수{} 안으로 들어오는 자료를 저장하는 변수) : array
//(2)반환값(함수{}가 종료되면서 *함수호출* 했던곳으로 반환하는 자료) : x
//(3)처리할명령어(함수{}가 실행되면서 실행할 코드들) : for 반복문을 사용하여 배열의 모든 요소를 콘솔에 하나씩 출력
...
isPositive(3); // [확인차 정의한 함수 호출], 함수를 실행하고 반환(return) 값
let result= isPositive(3); //[확인차 정의한 함수 호출 / 반환값 변수에 저장o]
console.log(result)
/*
예제 9: 전역 변수와 지역 변수
...
//[확인]
console.log(globalText);        //ok
console.log(localText);         //fail : 함수 안에서 선언된 지역변수 라서 오류 발생한다.
printLocal();// 함수호출