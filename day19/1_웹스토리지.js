/*
    [ 웹스토리지 ]
        - 웹브라우저 (개발자도구) -> [application]탭 -> 왼쪽 사이드메뉴(local/session)
        - 도메인(http://127.0.0.1:5500, https://www.naver.com/ 마다 웹스토리가 할당된다!
        1. 종류
    ...
    2. 사용법
        sessionStorage 객체
        localStorage 객체
        (1) session.Storage.setItem('속성명'/key , 값/value);
    ...
    [ JSON ] :  자바스크립트 객체 문법의 형식
        *  자바객체를 문자열타입 이지만 객체형식/모양으로 표현한것
        - 3 : 숫자타입 / 숫자모양       vs "3" : 문자열타입, 숫자모양
        - {age:40} : 객체타입, 객체모양 vs"{age:40}" : 문자열타입,객체모양
        - 왜?? 인터넷상의 데이터통신하는 http 는 객체지향이 아니다. 객체 누구세요?. 문자열과 바이트 전송 가능
...
sessionStorage.setItem('m1', JSON.stringify(['유재석',40]));    //[0], 대괄호 포함 한 저장
localStorage.setItem('m2', JSON.stringify({name:'유재석', age :40})); //[0], 중괄호 포함 한 저장
