/*[ 실습12 : 미니 가계부 페이지 ]
1. 목표
   - 사용자가 자신의 수입/지출 내역을 날짜, 항목, 금액으로 기록하고, 등록된 모든 내역을 한눈에 볼 
   수 있는 간단한 가계부 웹 페이지를 만듭니다.
2. 화면 구성
   - 페이지는 사용자가 정보를 입력하는 **'등록 영역'**과 그 결과를 보여주는 **'목록 영역'**으로 명확히
    구분되어야 합니다.
   - 등록 영역:
      - 날짜 선택: 사용자가 내역의 날짜를 달력에서 직접 선택할 수 있어야 합니다.
      - 항 입력: 사용자가 금액을 숫자로 입력할 수 있어야 합니다.
      - 등록 버목 입력: 사용자가 지출 또는 수입 항목의 내용을 텍스트로 자유롭게 입력할 수 있어야 
      합니다. (예: "점심 식사", "교통비")
      - 금액튼: 위 세 가지 정보를 입력한 후, 목록에 추가하기 위한 "등록" 버튼이 있어야 합니다.
   - 목록 영역:
...
3. 핵심 기능
   - 초기 데이터 표시:
      - 사용자가 페이지에 처음 방문했을 때, 가계부 사용법을 쉽게 이해할 수 있도록 두 개의 예시 내역이
       목록 표에 기본적으로 표시되어 있어야 합니다.
   - 내역 등록 기능:
      - 사용자가 '등록 영역'에 날짜, 항목, 금액을 모두 입력하고 "등록" 버튼을 클릭하면, 해당 내역이 
      '목록 영역' 표의 가장 아래쪽에 새로운 행으로 즉시 추가되어야 합니다.
   - 금액 자동 서식:
      - '목록 영역'에 표시되는 모든 금액은 사용자의 가독성을 위해 천 단위마다 쉼표(,)가 자동으로 
      표시되어야 합니다. (예: 4500이 입력되면 "4,500"으로 보여야 함)
...
/*
    ...
    [ 작업순서 ]
        ...
        3. (백) 함수(기능) 설계
            (1) '등록버튼' 클릭했을때 입력받은 3가지 정보를 배열에 저장
                함수명 :    (아무거나) '등록함수'
                매개변수 :  x 
                반환값 :    x
...
let 코드자동번호 = 가계부목록[마지막인덱스].코드// 2 // 현재 초기 배열의 마지막 객체의 코드번호 ;
...
function 전체조회함수(){ console.log('---전체조회함수 exe---');
    ...
    for(let index = 0; index <=가계부목록.length -1; index++){
        const obj = 가계부목록[index]; // index번재 객체(정보) 호출
        html += `<tr> 
                    <td> ${obj.날짜} </td> 
                    <td> ${obj.항목명}</td> 
                    <td> ${obj.금액}</td> 
