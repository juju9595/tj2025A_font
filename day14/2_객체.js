...
=========================================================================================
//1.{속성명 : 자료}
    //{name: '유재석', age: 40, city :'인천'};
//2. let/cost 변수명 ={}
const person={name: '유재석', age: 40, city :'인천'};
//3.
console.log(person);
*/
...
for(let index=0; index <= Object.keys(inventory).length -1; index++){
    ...
    if(inventory[key]<10) {//만약에 index번째 속성/key명 값의 10 미만이면
        console.log(key);    
    }
}
*/
...
let html =`<table>
 ...
 for(let index=0; index <= data.length -1; index++){
    //index는 0부터 마지막 인덱스까지 1씩증가
    const obj = data[index]; // index번째의 객체(동별 인구정보) 호출
        console.log(obj);
    //*(동별) 객체별 속성 값 확인
    console.log(obj.동별);
    console.log(obj. 세대수);
    console.log(obj["인구수(계)"]); console.log(obj["인구수(남)"]); console.log(obj["인구수(여)"]);
    //테이블 내용물
    html+=`<tr>
