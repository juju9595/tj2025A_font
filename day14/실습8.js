...
//(방법)1
const array = [productName, price, company];
//(방법)2: 객체일때
const obj1 ={'productName':productName, 'price' : price, 'company':company}
...
for(let i=0; i <=  cart.length -1; i++){
  ...
  for(let j=0; j <= productsInfo.length -1; j++)
    let nopb = productsInfo[j];

    if(product.id == nopb.id)
...
/*
문제 9: 투표 결과 집계하기
다음 votes 배열은 투표 결과를 나타냅니다. 각 후보가 몇 표를 받았는지 집계하여, 후보의 이름이 키이고 득표수가 
값인 객체를 만들어 콘솔에 출력하시오.
...
조건 2: 평점의 정수 부분만큼 꽉 찬 별(★)을, 10 - 정수 만큼 빈 별(☆)을 출력합니다. (예: 평점이 8.5이면 ★ 8개, 
☆ 2개)
...
for(let index=0; index<=webtoons.length-1; index++){
    const obj4 = webtoons[index];
    output += `<div> ${obj4.title}`;

    let black = parseInt(obj4.rating / 10);
    for(let star=1; star <=10; star ++){
      if(star <= black){output += '<span>★</span>'}
...
for(let index=0; index<=data.length -1; index++){
  ...
  console.log(obj.업소명);  console.log(obj.세대수); console.log(obj.소재지);
  console.log(obj.지정메뉴); console.log(obj.전화번호); console.log(obj.업태);
