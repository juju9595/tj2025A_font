...
for( i = 0; i <= 회원테이블.length -1; i++ ){ //i는 0부터 마지막인덱스(회원)까지 1씩 증가
    let 회원 = 회원테이블[i]; //1번재 회원(객체)조회

    let 구독상태 = false;
    let 구독중인상품코드 =  -1; // 상품코드가 가질수 없는 임의값
    let 구독활성화 = '';

    for(let j= 0; j <= 구독로그테이블.length -1; j++){ //j는 0부터 마지막인덱스(로그)까지 1씩 증가
        let 구독로그 = 구독로그테이블[j]; //j번째 구독로그(객체) 정보 조회
        if(회원.회원코드 == 구독로그.회원코드){//* 만약에 i번째 회원코드가 j번째 구독회원코드 와 같으면
            //(3)
            구독상태 = true; // 현재가 회원코드가 구독로그에 존재한다
            구독중인상품코드 = 구독로그.상품코드; // 현재 구독중인 상품코드를 기록한다.(i 번째의 회원의 구독중인 상품코드)
    ...
    if(구독상태 == true){//
        ...
        for(let index=0; index <=구독상품테이블.length -1; index++){
            ...
            if(상품정보.상품코드 == 구독중인상품코드){// index번째 구독상품코드 와 i번째 화면의 구독중인 상품코드 와 같으면
                html += `<ul>
