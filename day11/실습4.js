...
*/
let 섭씨온도 = prompt("섭씨온도를 입력하세요")
let 화씨 = Number(섭씨온도 * 9 / 5) + 32
console.log(`화씨 : ${화씨}`);
...
*/
let 년도 = prompt("태어난 년도를 입력하세요")
let 현재나이 = 2025 - Number(년도)
...
*/
let 키 = prompt("키를 입력하세요")
let 몸무게 = prompt("몸무게를 입력하세요")
let BMI = Number(몸무게) / (Number(키/100) * Number(키/100))  
console.log(`BMI :${BMI}`);
...
let result = 아이디 == 'admin' ? "관리자" : "일반 사용자" && 이메일 == 'admin@test.com' ? "관리자" : "일반 사용자"
...
let result2 = 점수 >= 90 == A && 점수 >=80 <90 == B && 점수 <= ==C;
...
let result3 = 나이 >= 20 && 나이 <= 29 ? "이벤트 대상입니다." : "이벤트 대상이 아닙니다."
...
let todo = prompt("새로운 할 일을 입력하세요"); // 입력받기, 입력받고 입력받은 값으로 다음 행동이 필요하므로 저장
