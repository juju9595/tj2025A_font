...
/*
    ...
    !=(값)같지 않다.        !==(값과자료형)같지않다.    "10" ===10 false
    
    // 4. 논리연산자 : 하나의 연산결과를 (논리) 값으로 반환, && / || 항상 2개 이상의 비교연산자 필요하다.
    /*
        && 이면서 이고 그리고 모두 and 조건, 조건이 모두 true 이며 true
        || 이거나 하나라도 또는 or 조건  , 조건이 하나라도 ture이면 ture * | : 엔터  + [시프트키] + 위에 원하기호\
        ! 부정반대 not 조건
            초등학생이 사탕이면서 초콜릿 먹는다. and조건, 2개 먹어야 true(충족)
...
let 결과 = ( 정수 % 2 == 1 ) && ( 정수 % 7 == 0 )           //  false and true =>false      false and true =>true      false and true =>false
console.log(`결과 :${결과}`)
*/
...
/*
    ...
    + : 오른쪽 데이터를 왼쪽에 더한 후에 대입/할당/놓기      , A = A + 3 vs A += 3
*/
let 변수 = 10;
변수 += 3;          // 변수 = 변수 + 3 ; (선택)
console.log(변수);  //13
