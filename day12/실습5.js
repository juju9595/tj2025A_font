...
*/
//조건 : 배열찾기함수(.indexOf() ), fruitList.indexOF( 새로운과일명 ) == -1
...
*/
//조건 : 주민등록번호[6] =='1' || =='3',  number[6] == 1,3 && number[7] == 2,4
//(1) 문자열도 배열자료 기반이므로 인덱스가 존재한다.
//(2) 위치는 1부터, 인덱스는 0부터, 7번째 숫자 --> 6번째 인덱스 뜻
/*
...
10,000원 미만: 할인 없음
*/
//조건 price>=50,000 && price>=30,000 , price<50,000 && 
//price>=10,000 , price<30,000 && price<10,000

/*
let price = Number(prompt('구매금액:'));
if( price>=50000 ){console.log('5% 할인');}
else if( price>=30000 ){console.log('5% 할인');}
else if( price>=10000){console.log('1% 할인');}
...
*/
/*
let month =prompt('월을 입력하세요(1~12):');
...
else if(month == '12' || month == '1' || month == '2'){console.log('가을');}
...
let num4 = Number(prompt('첫번째 수:'))
let num5 = Number(prompt('두번째 수:'))
let num6 = Number(prompt('세번째 수:'))
if( num4 > num5 > num6){console.log(`가장 큰 수:${num4}`);}
else if( num4 > num6 > num5){console.log(`가장 큰 수:${num4}`);}
else if( num5 > num4 > num6){console.log(`가장 큰 수:${num5}`);}
else if( num5 > num6 > num4){console.log(`가장 큰 수:${num5}`);}
else{console.log(`가장 큰 수:${num6}`);}
*/
...
if( (year % 4 == 0) || (year % 100 != 0) && (year % 400 == 0)){console.log(`${year}년은 윤년입니다.`);}
...
*/
//(1) 3개의 정수를 입력받는다           (변수3개(자료3개) vs 배열1개(자료3개)
//(2) 3개의 하나의 배열 자료로 표현하여 변수에 저장
//(3) 스왑(교체) : 두 자료들 간의 위치를 교체하는 방법, 한 위치의 하나만 저장이 가능하다.
/*
    A=10; B=20;         두 변수간의 자료를 교체하ㅣ오.
    A=B;        B의 자료를 A에 대입 *A의 기존 자료는 사라진다.(변수는 하나의 자료 저장한다.)

// (4) 오름차순 : 두 수 간의 작은 값을 앞에 나열, 두 수 비교하기.
// A > B 비교하여 A가 더 크면 A 와 B의 위치를 변경/스왑
...
let temp;       // 임시변수 선언
if( numList[0] > numList[1]){   //첫번째 값이 두번째 값보다 크면 (스왑/교체) // 내림차순은 부호 바꾸면 됌
...
*/
// 조건 : 플레이어가 이기는 경우의 수(3), 지는수(3), 비기는수(3) => 경우의수 9개
/*  (방법1 : 모든 경우의 수 판단)
    ...
    (방법1 : 모든 경우의 수 판단)
        ...
        1 == (0=1)%3
        1 == (1+1)%3
        2. 플레이어2 값의 + 1하고 나누기 3 했을때 나머지가 플레이어 1과 같으면 플레이어1 승리
...
사용자로부터 제외하고 싶은 과목명을 입력받아, courseList에 해당 과목이 존재하면 목록에서 삭제한 뒤 변경된 배열 전체를 
출력하세요. 만약 존재하지 않는 과목이면 '해당 과목은 신청 목록에 없습니다.'를 출력하는 프로그램을 작성하시오.
*/
let courseList = ['수학', '영어', '과학', '국어'];
let deleteList = prompt("제외하고 싶은 과목명:");
let index = courseList.indexOf(deleteList);
if(index !== -1){
